...
export type TextInputProps = SharedInputProps &
  Omit<InputHTMLAttributes<HTMLInputElement>, 'children' | 'prefix' | 'size'> & {
    variant?: 'text' | 'search'
    rows?: never
  }

export type TextareaInputProps = SharedInputProps &
...
export const Input = forwardRef<HTMLInputElement | HTMLTextAreaElement, InputProps>(
  function Input(props, ref) {
    ...
    const description =
      [suppliedDescription, errorId, hintId].filter(Boolean).join(' ') || undefined
    ...
    return (
      <div className={rootClass}>
        <label
          className={`dl-input__label${visuallyHideLabel ? ' dl-input__label--hidden' : ''}`}
          htmlFor={controlId}
        >
          <span>{label}</span>
          {required && (
            <span className="dl-input__required" aria-hidden="true">
              required
            </span>
          )}
        </label>
        <div className="dl-input__frame">
          {hasStartAdornment && (
            <span
              className="dl-input__adornment dl-input__adornment--start"
              aria-hidden={variant === 'search' && startAdornment == null ? 'true' : undefined}
            >
              {variant === 'search' && startAdornment == null ? (
                <SearchIcon size={16} />
              ) : (
                startAdornment
              )}
          ...
          {variant === 'textarea' ? (
            ...
            <input
              ...
              {...commonProps}
              type={variant === 'search' ? 'search' : (props as TextInputProps).type}
              ref={ref as React.ForwardedRef<HTMLInputElement>}
            />
          )}
          {endAdornment != null && (
            <span className="dl-input__adornment dl-input__adornment--end">{endAdornment}</span>
          )}
        </div>
        {(errorMessage || helperText || showCount) && (
          <div className="dl-input__meta">
            <span
              id={errorId ?? hintId}
              className={
                errorMessage ? 'dl-input__message dl-input__message--error' : 'dl-input__message'
              }
            ...
            {showCount && (
              <span className="dl-input__count" aria-live="polite">
                {length}
