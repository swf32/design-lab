...
import { ArrowDownIcon, AssetsIcon, CodeIcon, ComponentIcon, DirectoryIcon, MoreIcon, TokensIcon, type IconProps } from '../../../assets/icons'
...
export function SemanticTreeItem({
  ...
  useEffect(()=>{
    ...
    const close=(event:PointerEvent)=>{if(!rootRef.current?.contains(event.target as Node))setActionsOpen(false)}
  ...
  return (
    ...
    >
      {canExpand
        ? <button className="file-tree__chevron-button" type="button" aria-label={t(expanded?'tree.collapse':'tree.expand')} onClick={onSelect}><ArrowDownIcon className={expanded ? 'file-tree__chevron' : 'file-tree__chevron file-tree__chevron--collapsed'} size={12}/></button>
        : <span className="file-tree__chevron-spacer"/>}
      {canColor
        ? <ColorPicker className="file-tree__color-picker" label={`${t('tree.color')}: ${node.name}`} value={color} onChange={onColorChange} trigger={<EntityIcon size={16}/>}/>
        : <span className="file-tree__entity-icon" style={{color:color??undefined}}><EntityIcon size={16}/></span>}
      <button className="file-tree__label" type="button" onClick={onSelect}><span>{node.name}</span></button>
      {canAct&&<div className="file-tree__actions">
        <button
          ...
          aria-haspopup="menu"
          onClick={()=>{setActionsOpen((current)=>!current);onActionClick?.()}}
        ><MoreIcon size={16}/></button>
        {actionsOpen&&<div className="file-tree__actions-menu" role="menu">{actions??<span role="menuitem" aria-disabled="true">{t('tree.actionsSoon')}</span>}</div>}
      </div>}
    </div>
  )
}
