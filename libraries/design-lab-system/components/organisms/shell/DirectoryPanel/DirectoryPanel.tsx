...
import {
  SourceSelect,
  type SourceOption,
} from '../../../molecules/inputs/SourceSelect/SourceSelect'
...
export function DirectoryPanel({
  ...
  const [storedColors, setStoredColors] = useState<Record<string, string>>(() =>
    persistItemColors ? readStoredColors(colorStorageKey) : {},
  )
  useEffect(
    () =>
      setExpanded(
  ...
  useEffect(() => {
    if (persistItemColors && typeof window !== 'undefined')
      window.localStorage.setItem(colorStorageKey, JSON.stringify(storedColors))
  }, [colorStorageKey, persistItemColors, storedColors])
  ...
  const colorKey = (item: DirectoryTreeItem) =>
    `${activeProject?.id ?? 'no-source'}:${item.kind}:${item.path}`
  ...
  return (
    <aside
      className={`directory-panel${isResizing ? ' directory-panel--resizing' : ''}`}
      aria-label={t('directory.label')}
    >
      <header className="directory-panel__header">
        <SourceSelect
          sources={projects}
          activeSource={activeProject}
      ...
      <div className="directory-panel__toolbar">
        ...
        <button type="button" aria-label={t('directory.add')}>
          +
        </button>
      </div>
      ...
      {searchEnabled && (
        <div className="directory-panel__search">
          <Input
            ...
            size="small"
            fullWidth
            placeholder={searchPlaceholder ?? t('directory.searchPlaceholder')}
            value={query}
      ...
      <div className="file-tree" role="tree">
        ...
        {!treeLoading && activeProject && tree.length === 0 && (
          <p className="file-tree__status">{t('directory.empty')}</p>
        )}
        {!treeLoading && !activeProject && (
          <p className="file-tree__status">{t('directory.noProject')}</p>
        )}
        {!treeLoading &&
          activeProject &&
          tree.length > 0 &&
          ...
          visibleTree.length === 0 && (
            <p className="file-tree__status">{t('directory.noResults')}</p>
          )}
        {!treeLoading &&
