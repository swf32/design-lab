...
Для Design Lab-authored Project/Library верхнеуровневая структура задаётся canonical contract. В
existing repository те же module kinds подключаются versioned relative mounts, найденными
read-only onboarding; дизайнер не вводит absolute paths вручную. Модули автоматически ищут
известные типы сущностей внутри configured roots. Optional metadata может лежать рядом с сущностью,
но центральный entity registry не требуется.
...
Manifest-declared `*.stories.ts(x)` хранит story metadata и `renderStoryExample`. Каждый example содержит `label`, serializable `props` и только при намеренно несериализуемом сценарии точный authored `source` плюс дополнительные `imports`. Обычный handoff автоматически выводится из фактического React tree и форматируется единым structural printer независимо от исходной раскладки Story: короткий JSX остаётся компактным, а длинные props, arrays, objects и nested JSX получают стабильные отступы. Workbench загружает module по обнаруженному component directory без центрального application registry. Scanner статически разбирает TypeScript/TSX imports и возвращает четыре прямых набора связей: production `uses` / `usedBy` и example-only `examplesUse` / `usedInExamplesBy`. Type-only imports игнорируются; production dependencies вычитаются из example graph, чтобы одна и та же связь не дублировалась. Preview imports production Components становятся diagnostics, потому что preview обязан оставаться самостоятельной illustrative composition.

Reusable Component описывает повторяемую механику и composition slots, а не один product flow. Поэтому Library `Dialog` владеет modality, focus, dismissal, surface layout и content/footer slots, тогда как `CreateProjectDialog` является application-композицией `Dialog + Input + Button` и не регистрируется отдельной design-system entity. Любой modal/fullscreen/top-layer Component в Workbench сначала показывает launcher; открытая поверхность имеет Close/Escape/backdrop или явное завершающее действие и никогда не оставляет Workbench Back единственным выходом.

Соседний `ComponentName.playground.tsx` обнаруживается по соглашению об имени и использует общий typed control registry. Это исключительно wireframing/ideation artifact: он может существовать до production entry, поэтому wireframe-only Component не попадает в generated package barrel, но остаётся полноценной filesystem entity. Его route является отдельной fullscreen review surface: desktop rail + Canvas, mobile Canvas + вызываемый поверх него controls rail; background control закреплён сверху справа над Canvas. Production Component Playground на detail-странице имеет другой источник: реальный Story renderer плюс поддерживаемые manifest props, без switch по component id. Inspector автоматически строит copyable authored TSX/SCSS handoff: Component imports и manifest slots инструментируются во время review build, а raw elements связываются с исходными styles через Node analyzer. Ручные DOM markers и CSSOM matching не являются частью filesystem contract. Поддерживаемый lifecycle: `wireframe → in-progress → ready`; отсутствующий status не блокирует discovery и превращается в diagnostic.
...
| Minimal normalized model | stable ref, `path`, `type`, raw/resolved value, source format/location, optional `description`, `aliases[]`, `useWhen[]`, `avoidWhen[]`, `tags[]`, modes, reference chain, diagnostics       |
| File examples            | Native `value/type`, DTCG-style `$value/$type`, and supported legacy/plain JSON inside `tokens/**`                                                                                           |
| UX flow                  | Создать/изменить token → обновить превью и связи → использовать в components/pages                                                                                                           |
| Features                 | **MVP**: adapter-based read/normalize, scoped browse/search/get, references and diagnostics. **Next**: source-preserving CRUD/build. **Future**: custom adapters and cross-library overrides |

The filesystem document remains the source of truth. A format adapter detects and normalizes its
leaf syntax, references, groups and modes into a derived `TokenEntity`; it must preserve the
original path, syntax and source location and must not silently rewrite the document. Unsupported
or partially valid documents remain visible with scoped diagnostics instead of breaking the whole
Tokens module.

Semantic metadata is optional for compatibility with existing systems. Descriptions are expected
for semantic and component roles whose intent is not clear from their path; self-explanatory
primitives do not need comments that merely repeat a literal value. Supported inline descriptions,
aliases, usage guidance and tags feed the shared UI/MCP/CLI index and never become a second authored
registry. Mode overrides inherit base-token meaning. Colors describe roles such as surface,
readable text, selection, success, warning and danger rather than merely restating a hue or HEX.
Typography tokens describe how registered font families, sizes, weights and line heights are used.
`rules/TOKEN_RULES.md` defines the Design Lab-native authoring contract plus the supported read-only
compatibility dialects. Source-preserving create/update and explicit format migration remain future
write contracts.
...
| `variants[]`, `states[]`                                         | `string[]`                                | опционально                          | Только отображаемые списки. Production detail controls производны от `props`; Wireframe Playground controls — от registry внутри `*.playground.tsx`.                                                                                                                                                                                                                              |
...
| `family`                                                         | `string`                                  | опционально                          | Явно подтверждённая semantic Component Family для связи web/iOS/Android implementations. Совпадение имени или пути никогда не создаёт family молча.                                                                                                                                                                                                                                |
| `platform`, `technology`, `adapter`                              | `string`                                  | опционально                          | Разрешают только неоднозначность adapter detection; если факт выводится из `.vue`/`.svelte`/`.swift`/`@Composable`/Custom Element/TSX, поля не нужны.                                                                                                                                                                                                                               |
| `previewUrl`                                                     | `string`                                  | опционально                          | Внешняя browser-renderable preview surface для generic web/Go/Wasm targets. Даёт live preview/capture, но не обещает deep inspection.                                                                                                                                                                                                                                             |
