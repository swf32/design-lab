...
Component Card уже показывает имя компонента, source entry и количество variants. Authored preview не повторяет имя, категорию, manifest variants или другую metadata как собственный заголовок, badge или легенду; вся его площадь используется для узнаваемой иллюстрации.
