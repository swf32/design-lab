...
Inspector показывает source handoff как production `InspectorCodePopover` с `CodeBlock`. Component превращается в JSX с реальными public props, настоящий composition slot — в HTML fragment без внутренних `data-dl-*` markers, обычный element — в совпавшие authored same-origin CSS declarations. Обычный текстовый `children` не считается slot автоматически; slot marker принадлежит caller-supplied composition region наподобие `leading`, `trailing`, `header` или `footer`. Исходные `width: 100%`, shorthands и `var(--token)` сохраняются; computed pixel geometry и resolved RGB не выдаются за код. Click или keyboard activation копирует весь fragment.
