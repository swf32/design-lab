...
createServer(async (request, response) => {
  ...
  try {
    ...
    if (request.method === 'GET' && treeMatch) {
      return sendJson(response, 200, await getProjectTree(decodeURIComponent(treeMatch[1]), url.searchParams.get('module') ?? 'home'))
    }
    const sourceTreeMatch = url.pathname.match(/^\/api\/sources\/([^/]+)\/tree$/)
    if (request.method === 'GET' && sourceTreeMatch) {
      return sendJson(response, 200, await getProjectTree(decodeURIComponent(sourceTreeMatch[1]), url.searchParams.get('module') ?? 'home'))
    }
    const sourceModuleMatch = url.pathname.match(/^\/api\/sources\/([^/]+)\/modules\/([^/]+)$/)
    if (request.method === 'GET' && sourceModuleMatch) {
      return sendJson(response, 200, await getModuleEntities(decodeURIComponent(sourceModuleMatch[1]), decodeURIComponent(sourceModuleMatch[2])))
    }
    const sourceAssetMatch = url.pathname.match(/^\/api\/sources\/([^/]+)\/assets\/(.+)$/)
    if (request.method === 'GET' && sourceAssetMatch) {
      const asset = await getAssetFile(decodeURIComponent(sourceAssetMatch[1]), sourceAssetMatch[2].split('/').map(decodeURIComponent).join('/'))
      return sendBuffer(response, 200, asset.body, asset.contentType)
    }
    const sourceAssetPreviewMatch = url.pathname.match(/^\/api\/sources\/([^/]+)\/asset-previews\/(.+)$/)
    if (request.method === 'GET' && sourceAssetPreviewMatch) {
      const asset = await getAssetPreview(decodeURIComponent(sourceAssetPreviewMatch[1]), sourceAssetPreviewMatch[2].split('/').map(decodeURIComponent).join('/'))
      return sendBuffer(response, 200, asset.body, asset.contentType)
    }
    ...
    if (request.method === 'GET' && url.pathname === '/api/entities') {
      ...
      if (!projectId) return sendJson(response, 400, { error: { code: 'PROJECT_REQUIRED', message: 'projectId is required' } })
      const result = await getProjectTree(projectId, moduleId)
      return sendJson(response, 200, { revision, entities: result.tree })
    }

    if (!['GET', 'POST'].includes(request.method ?? '')) return sendJson(response, 405, { error: { code: 'METHOD_NOT_ALLOWED', message: 'Method not allowed' } })
