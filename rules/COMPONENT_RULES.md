...
When a Story needs to make an otherwise empty named slot visible, compose the production
`SlotPlaceholder` instead of drawing a local pink box. It owns the transparent dashed
`inspection.slot` boundary, canonical PlusIcon, and configurable CSS width/height. The placeholder
documents a slot location; it must not replace real product content or turn every ReactNode prop
into a slot. Slot identity still comes only from the consumer Component manifest.

Inspector output is a copyable source fragment, not a table of rendered geometry. Components and named slots produce the exact authored TSX callsite plus the static imports referenced by that callsite; a Library icon therefore hands off its canonical import and `<StarIcon />` usage, never its rendered SVG path. Ordinary elements resolve their source file through the automatic host-node registry, then ask the Node source analyzer for matching declarations from the imported `.scss` or `.css` file. The analyzer preserves authored `$variables`, `@use` / `@import`, mixin calls, nesting, shorthands, `width: 100%`, and `var(--token)` expressions. CSSOM, computed pixels, and resolved RGB values are never presented as source code. Clicking or keyboard-activating the code fragment copies the complete handoff source.
...
`WorkbenchAction` with `InspectorCodePopover`. Do not fork its source-registry or authored-style
inspection logic in an application view.
